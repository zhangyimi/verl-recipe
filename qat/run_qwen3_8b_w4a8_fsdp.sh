#!/usr/bin/env bash
# W4A8 Simulation: Weight FP4 + Activation FP8 (simulated via W4A16 kernel + FP8 noise)
set -euxo pipefail
current_dir="$(dirname "$(readlink -f "$0")")"

project_name='DAPO-NVFP4-QAT'
exp_name=${exp_name:-'DAPO-Qwen3-30B-A3B-W4A4'}
...
# QAT Configuration — W4A8 simulation
qat_enable=True
qat_mode=w4a8    # w4a8: weight FP4 training + FP8 activation fake-quant, vLLM uses W4A16 kernel
qat_config_path="${qat_config_path:-"${WORKING_DIR}/recipe/qat/config/nvfp4_w4a16.json"}"
# Enable FP8 activation simulation in vLLM inference
