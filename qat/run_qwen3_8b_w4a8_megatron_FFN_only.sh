#!/usr/bin/env bash
# 8B Qwen3 Dense - Megatron W4A8 QAT (requires patches/quantize_w4a8.patch applied), FFN-only (matches FSDP run_qwen3_w4a8_FFN_only.sh hyperparams)
set -xeuo pipefail
current_dir="$(dirname "$(readlink -f "$0")")"

project_name='DAPO-NVFP4-QAT'
exp_name=${exp_name:-"h100_8B_w4a8_megatron_FFN_$(date +%m%d)"}
...
# Must be ≥ (max_prompt_length + max_response_length) — verl asserts
# max_token_len > max_seq_len per individual sequence.
...
qat_enable=True
qat_mode=w4a8
qat_config_path="${qat_config_path:-"${WORKING_DIR}/recipe/qat/config/nvfp4_w4a8_megatron.json"}"
# Fallback flag (used if vLLM rollout side falls back to W4A16-pack + FP8 sim wrapper).
export VERL_W4A8_SIMULATION=1
